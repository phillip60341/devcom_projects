...
<div class="heading">
	...
	<div class="eventtitle">
		...
		<p id="gen" ><span style="font-family:bebas neue;font-size:60px;position:relative;left:0px;">ITSM</span> Seminar</p>
		<p id="year" >2016</p>
		<p id="uni" >UNIVERSITY OF MAKATI</p>
		<p id="col" >COLLEGE OF COMPUTER SCIENCE</p>
		<p id="grand">MAIN THEATER | 1:00 PM - 5:00 PM</p>
	...
	<div class="attendees">
		<p id="numero">
			<?php
			$countsql = "select count(*) from event3";
			...
			/
			<?php
			$countsql = "select count(*) from studenttbl where student_section like('%ITSM%')";
			$result = mysqli_query($connection,$countsql);
			$number = mysqli_fetch_assoc($result);
			echo($number["count(*)"]);
			?>
		</p>
		<p id="number">Number of Attendees: (ITSM)</p>
...
<div class="userlogin">
	...
	<div class="logform">
		...
		<form action = "" method = "post">
			<table>
				<tr>
					<td><label>Student Number (ITSM):</label></td>
				</tr>
				<tr>
					<td><input type="text" name = "student_id" maxlength = "9"></td>
				</tr>
				<tr>
				...
				<tr>
					<td><select name = "student_section">
						...
						<option value="III-BITSM" selected>III-BITSM</option>
...
<?php
	if(isset($_POST['btnsubmit'])){
		...
		$checksql = "SELECT * FROM studenttbl WHERE student_idnumber='$student_id' and student_section like('%ITSM%')";
		$checksqlresult = mysqli_query($connection,$checksql);
		//no record
		if(mysqli_num_rows($checksqlresult)==0){
			echo("<script>alert('There is no such student ID stored in the database, or you are from AppDev or NetAd. Approach the server about this problem or consider re-entering your student ID number.');window.location='index-c.php';</script>");
		}
		//have record
		else{
			
			//requires section to update
			/*$student_section = $_POST['student_section'];
			$addsection = "UPDATE studenttbl SET student_section='$student_section' WHERE student_idnumber='$student_id' ";
			mysqli_query($connection,$addsection);*/
			//end
			...
			$checksql1 = "select * from event3 where student_id='".$student_id."'"; 
			$checksqlresult1 = mysqli_query($connection,$checksql1);
			//pag hindi pa
			if(mysqli_num_rows($checksqlresult1)==0){
				$addquery = "insert into event3 values('".$student_id."',now())";
				mysqli_query($connection,$addquery);
				
				$row = mysqli_fetch_assoc($checksqlresult);
				
				echo("<script>alert('".$row['student_name'].", You have been registered. Thank you!');window.location='index-c.php';</script>");
			}
			//pag-nakalogin na
			else{
				echo("<script>alert('You already have been registered in into this event. If you think this is a mistake, approach the server for this problem.');window.location='index-c.php';</script>");
			}
		}
	}
?>
