	...
	<div class="logform">
		...
		<form action = "" method = "post">
			<table>
				...
				<tr>
					<td>
						<input type = "radio" name = "gender" value = "M" id = "rad_gender1" checked>Male</input>
						<input type = "radio" name = "gender" value = "F" id = "rad_gender2">Female</input>
					</td>
				</tr>
				<tr>
...
<?php
	if(isset($_POST['btnsubmit'])){
		...
		//something
		$checksql = "select * from studenttbl where student_idnumber='".$id_number."'";
		$checkresult = mysqli_query($connection,$checksql);
		if(mysqli_num_rows($checkresult)==0){
			$addsql = "insert into studenttbl(student_idnumber,student_name,student_section,student_bday,student_address,student_guardian,student_sex) values('".$id_number."','".$name."','".$section."','NOT SET','NOT SET','NOT SET','".$gender."')";
			mysqli_query($connection,$addsql);
			echo("<script>alert('Entry added to database');</script>");
			
			$addquery = "insert into event4 values('".$id_number."',now())";
			mysqli_query($connection,$addquery);
			echo("<script>alert('You have been registered. Thank you!');window.location='register.php';</script>");
		}
		else{
			echo("<script>alert('You are already registered in the CCS database');window.location='register.php';</script>");
		}
		//end something
		...
		// $addquery = "insert into event3 values('".$id_number."',now())";
		// mysqli_query($connection,$addquery);
